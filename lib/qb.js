...
function QB (options) {
  ...
  if (!qb._options.redis) {
    throw new Error('QB requires a redis connection')
  }

  qb._redis = qb._options.redis;
...
QB.prototype.undefer_remove = function undefer_remove(type, id, callback) {
  if (!this._redis.connected) {
    return callback(new Error('Redis is not connected. Cannot undefer right now.'))
  }
  this.queue(type).undefer_remove(id, callback)
  return qb
}

QB.prototype.undefer_push = function undefer_push(type, id, callback) {
  if (!this._redis.connected) {
    return callback(new Error('Redis is not connected. Cannot undefer right now.'))
  }
  this.queue(type).undefer_push(id, callback)
  return qb
}
...
function _listeners(qb, cb) {
  if (!qb._listening) {
    qb.on('push', function rediscatch(type, task, next) {
      next(qb._redis.connected ? null : new Error('Redis is not connected. Cannot push tasks.'))
    })
...
function _start(qb, types, callback) {
  var Qopt = qb._options.Q,
    ...
    readycount = _.keys(types).length + 2;
  ...
  async.parallel([
    ...
    async.apply(async.each, _.pairs(types), function (pair, cb) {
      var type = pair[0],
        ...
        options = _.extend(_.clone(qb._options), {prefix: key}, qb._options.specific[type] || {});

      if (options.allow_defer) { readycount++; }
      if (options.allow_recur) { readycount++; }

      var queue = new Q(qb._redis, options);

      if (options.allow_defer) { queue.on('deferred-ready', ready) }
      if (options.allow_recur) { queue.on('recurring-ready', ready) }

      var listener = queue.listen(qb._options)
        .on('ready', ready)
    ...
    _listeners.bind(null, qb)
  ], function (err) {
    ready(err)
  });

  qb._redis.ready ? ready() : qb._redis.on('ready', ready)

  function ready(err) {
    if (err) {
      readycount = -1 // shortcircuit
      callback(err)
    }
