...
var default_options = {
  aliases: {}
, catch_sigterm_end: true // Catch sigterm and call qb.end()
, end_timeout: 10000 // Timeout on an end() call
, name: '<unnamed>' // For logging and identification. Set to `qb.name`
}
...
QB.prototype.process = function (type, task, callback) {
  var qb = this
  if (!qb._types[type])
    return errcallback(qb, callback)(new Error('no ability to process task of type ' + type))
...
QB.prototype.end = function end(callback) {
  ...
  qb._ended = true;
  qb.log.info('Ending QB %s', qb.name);
  ...
  _end(qb, function (err) {
    ...
    qb.log.info('All tasks finished. QB %s Ended.', qb.name);
