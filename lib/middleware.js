...
_.each({pre: 'pre:', post: 'post:', on: ''}, function (prefix, name) {
...
MiddlewareProvider.prototype.emit = function emit(event) {
  var self = this,
    ...
    wares = (this._wares['pre:'+event] || [])
      ...
      .concat(this._wares['post:'+event] || []),
    next = createMiddlewareChain(wares, args, function (err) {
      if (err) {
        self.emit.apply(self, ['error', err].concat(Array.prototype.slice.call(arguments, 1)));
      }
    });
